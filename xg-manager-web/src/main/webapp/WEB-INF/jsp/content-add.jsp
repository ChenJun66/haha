...
<script type="text/javascript">
	...
	$(function(){
		...
		/* 初始化单张图片上传按钮，也就是说一次只能选择一张图片（因为我们的pic和pic2字段各自对应一张图片路径） */
		TT.initOnePicUpload();
		/* 把用户选择的内容分类节点的id赋值给隐藏域<input type="hidden" name="categoryId"/>。这样我们在提交表单的时候便把内容分类也一并提交了 */
		$("#contentAddForm [name=categoryId]").val($("#contentCategoryTree").tree("getSelected").id);
	});
	
	var contentAddPage  = {
			/* 当我们点击提交的时候便触发submitForm : function ()方法 */
			submitForm : function (){
				if(!$('#contentAddForm').form('validate')){
					/* 如果表单输入的不合法那么会提示"表单还未填写完！ */
					$.messager.alert('提示','表单还未填写完成!');
					return ;
				}
				/* 如果表单填写合法，那么富文本编辑器将与表单中的隐藏域字段进行内容同步 */
			...
			clearForm : function(){
				/* $('#contentAddForm').form('reset');的意思是将刚才表单中输入的内容清空 */
